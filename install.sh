...
run_inject() {
  ...
  if [ ! -f "package.json" ] && [ ! -f "Cargo.toml" ] && [ ! -f "go.mod" ] && [ ! -f "pyproject.toml" ] && [ ! -f "Makefile" ]; then
    warn "No package.json or project manifest found. Are you in a project root?"
  ...
  cp "$HARNESS_SRC/.claude/commands/new-feature.md"      .claude/commands/
  cp "$HARNESS_SRC/.claude/commands/debug.md"             .claude/commands/
  cp "$HARNESS_SRC/.claude/commands/design-system.md"     .claude/commands/
  cp "$HARNESS_SRC/.claude/commands/clarify-business.md"  .claude/commands/
  cp "$HARNESS_SRC/.claude/commands/quality-check.md"     .claude/commands/
...
SETTINGS_EOF
    success "Created minimal settings.json (run /init-project to customize)"
  else
  ...
  # Instead create a skeleton that /init-project will fill
  if [ ! -f "CLAUDE.md" ]; then
    cat > CLAUDE.md << 'SKELETON_EOF'
# CLAUDE.md

> This file was auto-generated by harness inject. Run `/init-project` in Claude Code to auto-detect your tech stack and fill in project-specific details.

## Project Overview

<!-- /init-project will fill this -->

## Quick Commands

<!-- /init-project will detect your package manager and scripts -->

```bash
# TODO: run /init-project to populate
```

## Code Conventions

<!-- /init-project will analyze your codebase and fill this -->
...
SKELETON_EOF
    success "Created skeleton CLAUDE.md (run /init-project to customize)"
  else
...
case "${1:-}" in
  ...
  --help|-h|"")
    ...
    echo -e "    ${GREEN}./install.sh --inject${NC}          Inject harness into existing project (any stack)"
    echo ""
